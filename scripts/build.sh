...
# Enable modules
    sed -i 's|# CONFIG_MODULES is not set|CONFIG_MODULES=y|g' .config || echo "Error enabling modules"

    # Add module unload support
    sed -i '/CONFIG_MODULES=y/a\CONFIG_MODULE_UNLOAD=y' .config || echo "Error adding module unload"

    # Enable debug info
    sed -i 's|# CONFIG_DEBUG_INFO is not set|CONFIG_DEBUG_INFO=y|g' .config || echo "Error enabling debug info"
...
EOL
    ...
    # Create initramfs
    cp /build/workspace/code/hellomod/hello_arm.ko /build/output/initramfs/lib/modules/
    cd /build/output/initramfs
